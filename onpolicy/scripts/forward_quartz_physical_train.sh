...
echo "Info: this is forward agent, experiment circuit is ${qasm_file_name} (\"reversed\" should not appear in the name)!"
echo "Info: device is ${device_name}, qasm_file_name is ${qasm_file_name}, reversed_qasm_file_name is ${reversed_qasm_file_name}"
    ...
    CUDA_VISIBLE_DEVICES=0,1,2,3 python train/train_quartz.py \
     ...
     \
     --two_way_mode "forward" --two_way_save_interval 100 --two_way_save_count 8 --two_way_clear_on_save \
